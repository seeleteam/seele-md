...
## 编辑合约
你可以基于任何支持solidity的编辑器编写合约。下述将基于[remix在线编辑器](http://remix.ethereum.org/#appVersion=0.7.7&optimize=true&version=soljson-v0.4.24+commit.e67f0147.js)为例，进行说明。注意使用与合约版本一致的编译器。
...
```
![注意：使用Seele部署合约，建议使用的solidity版本为0.4.24-0.4.26](./remixcompile.png)
...
### 本地测试环境
为保障提交到Seele主网的合约是可用的，建议先在本地运行[Seele私链节点](./../How-to-build-your-own-seele-network.md)，
并基于私链进行合约部署和调用测试。
### 主网环境
